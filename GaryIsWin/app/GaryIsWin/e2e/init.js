...
const { otherDetox, otherDevice } = require("./devices");
...
beforeAll(async () => {
  ...
  const detoxURLBlacklistRegex = getDetoxURLBlacklistRegexFromDomains([
    "ec2-18-188-87-120.us-east-2.compute.amazonaws.com",
  ]);
  const appConfig = {
    newInstance: true,
    launchArgs: {
  ...
  await detox.cleanup();
  await otherDetox.cleanup();
  await detox.init(config, { launchApp: false });
  await device.launchApp(appConfig);
  await otherDetox.init(config, { launchApp: false, initGlobals: false });
  await otherDevice.launchApp(appConfig);
}, 300000);
